...
export default function Commander(terminal) {
  ...
  this.passives = {
    find: p => {
      terminal.cursor.find(p.str);
    },
    select: p => {
      terminal.cursor.select(p.x, p.y, p.w, p.h);
    },
    inject: p => {
      terminal.cursor.select(p._x, p._y);
      terminal.source.inject(p._str, false);
    },
    write: p => {
      terminal.cursor.select(p._x, p._y, p._str.length);
    }
  };
  ...
  this.actives = {
    // Ports
    osc: p => {
      terminal.io.osc.select(p.int);
    },
    udp: p => {
      terminal.io.udp.select(p.int);
    },
    ip: p => {
      terminal.io.setIp(p.str);
    },
    cc: p => {
      terminal.io.cc.setOffset(p.int);
    },
    pg: p => {
      terminal.io.cc.stack.push({
    ...
    // Speed
    apm: p => {
      terminal.clock.setSpeed(null, p.int);
    },
    bpm: p => {
      terminal.clock.setSpeed(p.int, p.int, true);
    },
    time: p => {
      terminal.clock.setFrame(p.int);
    },
    rewind: p => {
      terminal.clock.setFrame(terminal.orca.f - p.int);
    },
    skip: p => {
      terminal.clock.setFrame(terminal.orca.f + p.int);
    },
    // Effects
    rot: p => {
      terminal.cursor.rotate(p.int);
    },
    // Themeing
    color: p => {
      terminal.theme.set("b_med", p.parts[0]);
      terminal.theme.set("b_inv", p.parts[1]);
      terminal.theme.set("b_high", p.parts[2]);
    },
    graphic: p => {
      terminal.theme.setImage(
        terminal.source.locate(
          p.parts[0] + "." + (p.parts[1] ? p.parts[1] : "jpg")
        )
      );
    },
    // Edit
    ...
    inject: p => {
      terminal.cursor.select(p._x, p._y);
      terminal.source.inject(p._str, true);
    },
    write: p => {
      terminal.cursor.select(p._x, p._y, p._str.length);
      terminal.cursor.writeBlock([p._str.split("")]);
    }
  };
  ...
  this.onKeyDown = function(event) {
    ...
    if (event.key === " " && terminal.cursor.mode === 1) {
  ...
  this.onArrowLeft = function(mod = false, skip = false, drag = false) {
    ...
    console.log(terminal.cursor);
  };

  this.onArrowRight = function(mod = false, skip = false, drag = false) {
    const leap = skip ? terminal.grid.w : 1;
    // mod is whether or not we are modifying the cursor window size
    // skip is (I think) whether we are going one or 8 (or whatever the grid
    // size is)?
    // drag is whether the option key is held, in which case we drag our
    // contents
    terminal.toggleGuide(false);
    if (drag) {
      terminal.cursor.drag(leap, 0);
