...
export default function Terminal() {
  ...
  this.isNear = function(x, y) {
    return (
      x > parseInt(this.cursor.x / this.grid.w) * this.grid.w - 1 &&
      x <= (1 + parseInt(this.cursor.x / this.grid.w)) * this.grid.w &&
      y > parseInt(this.cursor.y / this.grid.h) * this.grid.h - 1 &&
      y <= (1 + parseInt(this.cursor.y / this.grid.h)) * this.grid.h
    );
  };
  ...
  this.isLocals = function(x, y) {
    return (
      this.isNear(x, y) === true &&
      (x % (this.grid.w / 4) === 0 && y % (this.grid.h / 4) === 0) === true
    );
  };
  ...
  this.makeTheme = function(type) {
    // Operator
    if (type === 0) {
      return { bg: this.theme.active.b_med, fg: this.theme.active.f_low };
    }
    // Haste
    if (type === 1) {
      return { fg: this.theme.active.b_med };
    }
    // Input
    if (type === 2) {
      return { fg: this.theme.active.b_high };
    }
    // Output
    if (type === 3) {
      return { bg: this.theme.active.b_high, fg: this.theme.active.f_low };
    }
    // Selected
    if (type === 4) {
      return { bg: this.theme.active.b_inv, fg: this.theme.active.f_inv };
    }
    // Locked
    if (type === 5) {
      return { fg: this.theme.active.f_med };
    }
    // Reader
    if (type === 6) {
      return { fg: this.theme.active.b_inv };
    }
    // Invisible
    if (type === 7) {
      return {};
    ...
    // Reader
    if (type === 8) {
      return { bg: this.theme.active.b_low, fg: this.theme.active.f_high };
    }
    // Reader+Background
    if (type === 10) {
      return { bg: this.theme.active.background, fg: this.theme.active.f_high };
    }
    // Default
  ...
  this.drawInterface = function() {
    ...
    if (this.commander.isActive === true) {
      this.write(
        `${this.commander.query}${this.orca.f % 2 === 0 ? "_" : ""}`,
        col * 0,
        this.orca.h + 1,
        this.grid.w * 4
      );
      ...
      this.write(
        `${this.cursor.x},${this.cursor.y}${this.cursor.mode === 1 ? "+" : ""}`,
        col * 0,
        this.orca.h + 1,
        this.grid.w,
        this.cursor.mode === 1 ? 1 : 2
      );
      this.write(
    ...
    this.write(
      `${this.grid.w}/${this.grid.h}${
        this.tile.w !== 10 ? " " + (this.tile.w / 10).toFixed(1) : ""
      }`,
      col * 1,
      this.orca.h,
      this.grid.w
    ...
    this.write(
      ...
      this.orca.h,
      this.grid.w,
      this.source.queue.length > terminal.orca.f ? 3 : 2
    );
    this.write(
      `${this.clock}`,
      col * 3,
      this.orca.h,
      this.grid.w,
      this.clock.isPuppet === true > -1 ? 3 : 2
    );
    ...
    if (this.orca.f < 15) {
      ...
      this.write(
        `${this.io.inspect(this.grid.w)}`,
        col * 4,
        this.orca.h,
        this.grid.w
      );
      this.write(
        `${display(variables, this.orca.f, this.grid.w)}`,
        col * 4,
        this.orca.h + 1,
        this.grid.w
      );
  ...
  this.drawGuide = function() {
    ...
    const operators = Object.keys(this.library).filter(val => {
      return isNaN(val);
    });
  ...
  this.fit = function() {
    const size = {
      w: this.orca.w * this.tile.w + 60,
      h: this.orca.h * this.tile.h + 60 + 2 * this.tile.h
    };
    const win = require("electron").remote.getCurrentWindow();
    const winSize = win.getSize();
    const current = { w: winSize[0], h: winSize[1] };
    if (current.w === size.w && current.h === size.h) {
      console.warn("Terminal", "No resize required.");
      return;
    }
    console.log(
      "Source",
      `Fit terminal for ${this.orca.w}x${this.orca.h}(${size.w}x${size.h})`
    );
  ...
  this.resize = function(force = false) {
    ...
    this.el.style.height = `${Math.ceil(
      (this.tile.h + this.tile.h / 5) * this.orca.h
    )}px`;

    this.context.textBaseline = "bottom";
    this.context.textAlign = "center";
    this.context.font = `${this.tile.h *
      0.75 *
      this.scale}px input_mono_medium`;
  ...
  this.crop = function(w, h) {
    ...
    if (h > this.orca.h) {
      block = `${block}${`\n${".".repeat(this.orca.w)}`.repeat(
        h - this.orca.h
      )}`;
    } else if (h < this.orca.h) {
      block = `${block}`
        .split("\n")
        .slice(0, h - this.orca.h)
        .join("\n")
        .trim();
    }
    ...
    if (w > this.orca.w) {
      block = `${block}`
        .split("\n")
        .map(val => {
          return val + ".".repeat(w - this.orca.w);
        })
        .join("\n")
        .trim();
      ...
      block = `${block}`
        .split("\n")
        .map(val => {
          return val.substr(0, val.length + (w - this.orca.w));
        })
        .join("\n")
        .trim();
    }
  ...
  this.docs = function() {
    let html = "";
    const operators = Object.keys(library).filter(val => {
      return isNaN(val);
    });
    for (const id in operators) {
      const oper = new this.library[operators[id]]();
      const ports = oper.ports.input
        ? Object.keys(oper.ports.input).reduce((acc, key, val) => {
            return acc + " " + key;
          }, "")
        : "";
      html += `- \`${oper.glyph.toUpperCase()}\` **${oper.name}**${
        ports !== "" ? "(" + ports.trim() + ")" : ""
      }: ${oper.info}.\n`;
    }
    return html;
