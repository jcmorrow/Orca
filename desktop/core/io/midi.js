...
export default function Midi(terminal) {
  ...
  this.push = function(
    ...
    for (const id in this.stack) {
      if (
        this.stack[id].channel === channel &&
        this.stack[id].octave === octave &&
        this.stack[id].note === note
      ) {
  ...
  this.update = function() {
    ...
    terminal.controller.add(
      "default",
      "Midi",
      `MIDI Send Clock ${this.isClock === true ? " — On" : " — Off"}`,
      () => {
        this.toggleClock();
        this.update();
      },
    ...
    terminal.controller.add("default", "Midi", `Refresh Device List`, () => {
    ...
    if (this.outputs.length < 1) {
      ...
      for (const id in this.outputs) {
        terminal.controller.add(
          "default",
          "Midi",
          `${this.outputs[id].name} Output ${
            terminal.io.midi.outputIndex === parseInt(id) ? " — Active" : ""
          }`,
          () => {
      ...
      terminal.controller.add(
        "default",
        "Midi",
        `No Output ${terminal.io.midi.outputIndex === -1 ? " — Active" : ""}`,
        () => {
    ...
    if (this.inputs.length < 1) {
      ...
      for (const id in this.inputs) {
        terminal.controller.add(
          ...
          `${this.inputs[id].name} Input ${
            terminal.io.midi.inputIndex === parseInt(id) ? " — Active" : ""
          }`,
          () => {
      ...
      terminal.controller.add(
        "default",
        "Midi",
        `No Input ${terminal.io.midi.inputIndex === -1 ? " — Active" : ""}`,
        () => {
